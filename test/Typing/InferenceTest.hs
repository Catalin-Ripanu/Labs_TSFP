...
test_infer = do
  ...
  where
    inferInternal expr local global = 
        case infer (internal expr) local global emptySubst 0 of
            Right typ -> typ
            Left err -> error $ "Type inference failed: " ++ err
    inferInternalError expr local global = 
        case infer (internal expr) local global emptySubst 0 of
            Right _ -> False
            Left _ -> True
